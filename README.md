...
### Numero di fold e procedure di ricampionamento
...
Le medie seguono in larga parte la numerosità della parte di addestramento, che è il compromesso fra distorsione e varianza delle procedure di ricampionamento misurato sui dati del progetto: l'esclusione di un motore per volta addestra su 99 unità e dà la media più bassa su FD001, il bootstrap lascia fuori dal campione circa un terzo dei motori e dà la più alta su entrambi i sottoinsiemi, il K-Fold a 10 sta sotto quello a 5. La partizione unica non rispetta l'ordinamento, perché addestra su 70 motori e su FD003 produce comunque la media più bassa della tabella; è però anche la procedura la cui media è meno determinata, per la ragione riportata subito sotto. Le dispersioni non misurano la stessa quantità e non sono intercambiabili: per la partizione unica descrivono la variabilità fra partizioni, per il K-Fold la variabilità fra parti di verifica, per il bootstrap la variabilità fra campioni; quella dell'esclusione di un motore per volta è la più ampia perché ogni stima è calcolata su una singola traiettoria e misura quanto i motori differiscono fra loro. Il K-Fold a 10 ha dispersione doppia rispetto a quello a 5 con media quasi identica, ed è la giustificazione empirica del numero di fold adottato.
...
| PCA, K-Means, clustering gerarchico, silhouette, indice di Rand corretto | 12 |

Le due righe che non corrispondono a modelli in confronto sono coperte altrove nel documento: le procedure di ricampionamento sono il protocollo che regge il confronto e sono confrontate fra loro nella sezione sul numero di fold, i metodi non supervisionati rientrano come strumenti di esplorazione e di commento nella sezione sulla struttura delle due popolazioni, perché il task del progetto è di regressione.
...
L'ampiezza interna alla famiglia lineare vale 0,03 cicli su FD001 e 0,09 su FD003, contro dispersioni mediane di 1,18 e 1,45: otto modelli diversi producono lo stesso numero. La famiglia a margine ha invece l'ampiezza interna maggiore, perché contiene sia il modello migliore del confronto sia il peggiore, e la scelta del kernel vi conta più della scelta della famiglia.
...
Cautela che vale per l'intera tabella: i punteggi sono ottimisticamente distorti perché la cross-validation non è annidata, e la distorsione non è uniforme fra le righe, perché il numero di configurazioni valutate sulle stesse partizioni su cui il punteggio è poi riportato cambia di ordini di grandezza da un modello all'altro.
...
**L'arresto anticipato della rete resta disattivato per una ragione di protocollo.** La partizione interna che la libreria costruirebbe è ottenuta mescolando le righe, quindi collocherebbe cicli adiacenti dello stesso motore da entrambe le parti, reintroducendo dentro il modello la contaminazione che il vincolo di gruppo esclude fuori. Il numero massimo di iterazioni entra invece in griglia, a differenza del numero di alberi del blocco precedente, perché la curva misurata prima dell'esecuzione mostra che le due quantità non si comportano allo stesso modo. Fra 100 e 3.000 iterazioni, sull'architettura a due strati con passo 1e-3, la perdita di addestramento scende da 114,0 a 74,0 su FD001 e da 77,3 a 45,2 su FD003, mentre l'errore sulla parte di verifica della partizione sale da 15,86 a 18,87 cicli e da 14,89 a 17,89. Sull'architettura a uno strato, con lo stesso passo, lo stesso intervallo lascia l'errore fermo su FD001 (da 16,08 a 16,16) e lo fa scendere su FD003 (da 16,31 a 15,28). Le iterazioni non fanno quindi saturare l'errore come il numero di alberi: dove la capacità è alta lo fanno risalire, dove è bassa no. Governano un compromesso che dipende dalle altre scelte della griglia, e vanno selezionate insieme a esse.

![Perdita di addestramento ed errore di verifica del percettrone al crescere delle iterazioni](results/figures/convergenza_rete.png)
...
Due configurazioni restano selezionate su un estremo della griglia anche dopo l'estensione, e su entrambe la catena si chiude per movimento insufficiente: la penalizzazione minima del kernel lineare su entrambi i sottoinsiemi, dove il profilo lungo quell'asse è piatto su quattro decadi, e l'architettura più capiente della rete su FD003, dove lo stesso profilo è ripido dal lato stretto e piatto in cima.
...
Sono tre, segnalate come tali anche nel punto in cui compaiono. La **cross-validation con vincolo di gruppo** è la trasposizione diretta del K-Fold a dati raggruppati, resa obbligatoria dalla struttura del dataset. Il **confronto appaiato fold per fold** e il **conteggio delle inversioni fra coppie separabili** sono letture descrittive introdotte per esaminare il vertice della graduatoria e la concordanza fra cross-validation e verifica; nessuna delle due produce statistiche test, e nessuna sostituisce la regola di lettura del protocollo.
