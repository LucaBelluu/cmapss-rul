...
Ruolo nel progetto
    ...
    linearità. Non contiene stimatori né logica di valutazione: gli stimatori
    sono nel registro, la valutazione nel protocollo.

Cosa riceve
    Pipeline già adattate, oppure una pipeline non adattata e un insieme di
...
Le due misure di importanza
    ...
    La riduzione di impurità è calcolata durante la costruzione dell'albero:
    somma quanto ciascuna variabile ha ridotto l'errore quadratico nei nodi in
    cui è stata usata, pesando per il numero di osservazioni che li
    attraversano. È gratuita perché è un sottoprodotto dell'addestramento, ed
    è distorta verso le variabili con molti valori distinti, che offrono più
    punti di taglio fra cui scegliere il migliore. Su questo dataset la
    distorsione ha un bersaglio preciso, il numero di ciclo, che è un conteggio
    e assume perciò più valori distinti di qualunque lettura di sensore.
    Inoltre è calcolata sui dati di addestramento: una variabile può risultare
    importante per come l'albero è cresciuto e non per quanto serve a predire.
    ...
    sull'insieme di verifica; qui l'insieme di verifica ufficiale è chiuso fino
    ...
    ripartisce, e per la permutazione si annulla, perché mescolare una variabile
    lascia al modello l'informazione contenuta nelle sue sostitute. Una
    importanza bassa non è quindi una prova che la variabile sia inutile.

Unità dell'importanza per permutazione
    La misura è calcolata sulla metrica di riferimento del progetto e riportata
    ...
    errore, ed è leggibile accanto ai valori della tabella di confronto.

Convenzioni delle librerie sull'importanza per riduzione di impurità
    Scikit-learn restituisce importanze già normalizzate a somma uno.
    L'implementazione esterna di gradient boosting non normalizza, e per
    impostazione predefinita restituisce il guadagno medio per divisione anziché
    il guadagno totale: sotto lo stesso nome le due librerie riporterebbero
    quantità diverse. Il registro chiede il guadagno totale, che è l'analogo
    della grandezza di scikit-learn, e questo lettore normalizza comunque tutte
    le colonne a somma uno.

    Il bagging è l'unico insieme di scikit-learn che non espone affatto
    l'importanza delle variabili, mentre foresta, AdaBoost e gradient boosting la
    espongono. Viene perciò ricostruita qui come media non pesata delle
    importanze dei suoi alberi, che è la definizione stessa usata dalla foresta:
    ...
    AdaBoost fa media pesata per il peso di ciascuno stadio anziché semplice.
    La differenza è propria del modello e non viene uniformata: la quantità
...
# Metrica su cui è misurata l'importanza per permutazione. È la stessa su cui
...
def _importances_from_members(model, n_features: int) -> np.ndarray:
    ...
    `estimators_features_` identifica: i contributi vanno perciò riportati sulle
...
def impurity_importances(pipeline: Pipeline, feature_names: list[str]) -> pd.DataFrame:
    """Importanza per riduzione di impurità, normalizzata a somma uno.

    La colonna `zero` individua le variabili che il modello non ha mai usato per
    una divisione, ed è la stessa convenzione con cui negli altri blocchi si
...
def permutation_importances(
    ...
    *,
    ...
    addestramento e l'importanza è misurata sulla parte di verifica, che non ha
    partecipato all'addestramento. Le partizioni passate sono quelle del seme di
    ricerca: la misura non è una stima di prestazione e non entra in
    graduatoria, quindi non richiede le quindici partizioni di confronto.

    Il parallelismo è disattivato per impostazione predefinita. La funzione di
            ...
            pd.DataFrame(
                {
                    ...
                    "fold": split.fold,
                    "feature": features,
                    # Il punteggio è orientato in modo che valori maggiori
                    # siano migliori, quindi la differenza fra punteggio di
                    # riferimento e punteggio dopo la permutazione è già
...
def tree_summary(pipeline: Pipeline) -> dict:
    """Dimensione dell'albero potato: nodi, foglie, profondità.

    Sono le quantità con cui si legge quanto la potatura ha ridotto l'albero
