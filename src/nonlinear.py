"""Modelli del superamento della linearità: espansioni di base e modello additivo.
...
Cosa riceve
    ...
    funzioni di lettura ricevono una pipeline già adattata e l'elenco dei nomi
...
Perché le tecniche sono applicate a tutte le variabili
    Il laboratorio le presenta su una variabile per volta a scopo didattico,
    perché la curva stimata sia disegnabile. Un modello costruito su una sola
    variabile non sarebbe confrontabile con quelli degli altri blocchi, che
    usano l'intera matrice: qui l'espansione è applicata a ogni variabile e i
    termini risultanti entrano insieme in una regressione lineare, che è la
    forma in cui il laboratorio stesso usa `SplineTransformer` quando passa a
    più variabili.

Collocazione dei punti di taglio
    Per le step functions e per le spline la posizione dei tagli è un
    iperparametro con due valori: intervalli di ampiezza uguale sull'escursione
    della variabile, che è la costruzione del laboratorio, oppure tagli sui
    quantili. La differenza non è cosmetica: le letture dei sensori non sono
    distribuite uniformemente sul proprio intervallo, e con tagli equispaziati
    gli intervalli estremi possono contenere pochissime osservazioni, così che
    ...
    misura, e il costo aggiuntivo è di pochi secondi.
...
Adattatore per il modello additivo
    ...
    è l'adattatore minimo che risolve il problema: eredita da `BaseEstimator`,
    espone gli iperparametri del modello come argomenti del costruttore e
    costruisce i termini dentro `fit`, quando il numero di colonne è noto.
...
# modello additivo. È una risoluzione di disegno: non influisce sulla stima.
...
class GamRegressor(BaseEstimator, RegressorMixin):
    ...
    lam
        Intensità della penalizzazione sulla curvatura, comune a tutti i
        termini. Valori grandi riportano ciascun termine verso una retta.
    n_splines
        Numero di funzioni di base per termine: fissa quanto ogni funzione può
        essere flessibile prima che la penalizzazione intervenga.
    spline_order
        Grado delle funzioni di base. Resta 3 e non entra in griglia: è il
        valore del laboratorio, e la flessibilità è già governata dai due
...
def polynomial_estimator() -> Pipeline:
    ...
    L'espansione comprende le interazioni, che è la forma predefinita dello
    strumento usato nel laboratorio. È anche ciò che distingue questo modello
...
class QuietKBinsDiscretizer(KBinsDiscretizer):
    ...
    avviso. La condizione è attesa su questi dati (`sensor_10` assume quattro
    valori su FD003, e la censura del target rende quasi degenere la coda
    superiore di alcuni sensori), e l'esito è corretto: la variabile riceve
    meno colonne, le altre non sono toccate.

    L'avviso viene perciò filtrato. Non è una soppressione silenziosa: il
    numero di colonne effettivamente generate è registrato negli artefatti
    dell'esperimento, ed è da lì che si legge quanti intervalli sono stati
    rimossi. La differenza rispetto al lasciare l'avviso attivo è che una
    ...
    Il filtro è dentro `fit` e non a livello di modulo: uno scikit-learn che
...
def step_estimator() -> Pipeline:
    ...
    La discretizzazione è adattata dentro la pipeline, quindi i punti di
...
def spline_estimator() -> Pipeline:
    ...
    È la regression spline del laboratorio: le funzioni di base sono generate
    dallo strumento e i coefficienti stimati per minimi quadrati, senza
    penalizzazione sulla curvatura. La flessibilità è governata dal numero di
...
def _source_variable(term: str, feature_names: list[str]) -> str:
    ...
    L'attribuzione avviene per prefisso più lungo e non per contenimento:
    ...
    Il termine che combina due variabili è etichettato come interazione:
    attribuirlo a una delle due sarebbe arbitrario, e la quota di modello che
    sta nelle interazioni è una delle letture del blocco.
    """
    sources = set()
...
def expansion_terms(pipeline: Pipeline, feature_names: list[str]) -> pd.DataFrame:
    ...
    uno, perché descrivono l'effetto di una funzione di base e non di una
    variabile. La lettura utile è aggregata: sommando le ampiezze dei termini
    ...
    modelli lineari. La tabella conserva il dettaglio per termine, così che
...
def gam_terms(pipeline: Pipeline, feature_names: list[str]) -> pd.DataFrame:
    ...
    Per ciascun termine sono riportati i gradi di libertà effettivi, che
    misurano quanta flessibilità la penalizzazione ha lasciato alla funzione
    ...
    di significatività calcolato dalla libreria.

    L'escursione è la quantità confrontabile fra variabili, perché è nelle
    unità del target. I gradi di libertà effettivi ne descrivono la forma e
    non l'ampiezza: una variabile può avere una funzione molto curva e un
    effetto piccolo.
    """
    ...
    # La colonna `term` esiste anche qui, con il nome della variabile, perché
...
def gam_partial_dependence(pipeline: Pipeline, feature_names: list[str]) -> pd.DataFrame:
    ...
    È la lettura con cui il laboratorio commenta il modello additivo: per ogni
    variabile, la forma dell'effetto sul target. La griglia è generata dalla
    libreria sul dominio osservato della variabile.

    Le ascisse sono nella scala standardizzata, perché la standardizzazione è
    dentro la pipeline e il modello vede le variabili già trasformate. La
